...
async function main() {
  ...
  // Fetch with retry. A *successful but empty* result is almost always transient
  // (Shopify eventual-consistency mid-edit, a product briefly unpublished from
  // the Headless channel, or a throttle that returns empty data) — not a real
  // "the shop has no products" state, since sold one-of-ones still come back as
  // available:0. Retry a few times before trusting a zero.
  const RETRIES = 3;
  let products = [];
  let dropped = 0;
  for (let attempt = 1; attempt <= RETRIES; attempt++) {
    ...
    products = vendorMatched.filter((p) => VALID_CATEGORIES.has(p.category));
    dropped = vendorMatched.length - products.length;
    console.log(`Attempt ${attempt}/${RETRIES}: mapped ${products.length} product(s)` +
      (dropped ? ` (excluded ${dropped} vendor product(s) with no valid category tag).` : "."));
    if (products.length > 0) break;
    if (attempt < RETRIES) {
      console.warn("  Zero products returned — retrying in 5s (transient Shopify/channel blip?)...");
      await new Promise((r) => setTimeout(r, 5000));
    }
  }

  // FAIL-SAFE: never publish an empty catalog. A confirmed zero here means
  // something is wrong upstream (products unpublished from the Headless channel,
  // vendor/tag mismatch, token/scope issue, or a Shopify outage). Aborting the
  // build leaves the LAST GOOD deploy live instead of blanking the storefront.
  // Set ALLOW_EMPTY_CATALOG=1 to override (e.g. a genuine full teardown).
  if (products.length === 0 && process.env.ALLOW_EMPTY_CATALOG !== "1") {
    throw new Error(
      "Refusing to deploy an EMPTY catalog: Shopify returned 0 Barbermatic " +
      "products with a valid category tag after " + RETRIES + " attempts. " +
      "The current live site is kept. Check: products still published to the " +
      "Headless channel, Vendor == \"" + VENDOR + "\", and category:<slug> tags. " +
      "Override with ALLOW_EMPTY_CATALOG=1 if this is intentional."
    );
  }
